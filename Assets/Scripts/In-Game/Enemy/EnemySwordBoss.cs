...
public class EnemySwordBoss :EnemySword {
    protected override void Start() {
        base.Start(); // Llamar al método Start() de EnemySword

        // Personaliza el "boss"
        _speed = 0.4f;  // Más lento que el enemigo normal
        enemyHealth = 60f;  // Más vida que un enemigo normal
        _damage = 10f;  // Más daño
        _attackRange = 2f;
    ...
    public override void TakeDamage(float damageAmount, Vector2 knockbackDirection, float knockbackDistance) {
        ...
        if(enemyHealth <= 0) {
            ...
            playerScript.IncreaseCoins(10);  // Recompensa más grande por matar al boss
        } else {
